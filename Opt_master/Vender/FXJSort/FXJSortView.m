...
//创建titleList
-(void)firstTitleBtns:(NSMutableArray *)arr{

    self.backgroundColor = [UIColor whiteColor];
    
//    self.lineImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, kWidth, 0.5)];
//    self.lineImageView.backgroundColor = SEPARATOR_LINE_COLOR;
...
//
//    self.lineImageView.frame = CGRectMake(0, 2*firstSectionHeght, SCREEN_WIDTH, 0.5);

    
    
    self.secondSectionArr = [[NSMutableArray alloc]init];
...
-(void)secondTitleBtns:(NSArray *)arr{
    for(id obj in arr)
    {
        [self.secondSectionArr  addObject: obj];
    }
    for (int i = 0; i < arr.count; i++) {
...
-(void)NextSection{
    ...
    CGFloat firstSectionHeght = row * (buttonHeight+distance);
    self.lineImageView.frame = CGRectMake(0, firstSectionHeght, SCREEN_WIDTH, 0.5);
    NSLog(@" ===== %ld",self.newtitleArr.count);
    for (int i = 0; i< self.secondSectionArr .count; i++) {
            MyButton *button = [self viewWithTag:i+100];
            //button现在的位置
            NSInteger nowWeiZhi=self.secondSectionArr .count-i;
...
-(void)btnUp:(MyButton *)sender{
    ...
    for (NSInteger i = sender.tag; i<self.secondSectionArr .count+100; i++) {
        MyButton *button = [self viewWithTag:i+1];
        button.tag = i;
...
-(void)touchBtn:(MyButton *)sender{
    if (sender.isDown == NO) {
        ...
        sender.tag = self.secondSectionArr .count+100;
        NSString *title = sender.titleLabel.text;
        [self.secondSectionArr  insertObject:title atIndex:0];
        [self NextSection];
        
                sender.layer.cornerRadius=3;
                sender.layer.borderColor=PRIMARY_COLOR.CGColor;
        
    }else{
        NSInteger weiZhi=self.secondSectionArr .count-sender.tag+100-1;
        NSString *str = [self.secondSectionArr  objectAtIndex:weiZhi];
        [self.secondSectionArr  removeObjectAtIndex:weiZhi];
        [self.newtitleArr addObject:str];
        [sender setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
        sender.backgroundColor = PRIMARY_COLOR;
        sender.layer.cornerRadius=3;
        sender.layer.borderColor=PRIMARY_COLOR.CGColor;
        [self btnUp:sender];
    }
