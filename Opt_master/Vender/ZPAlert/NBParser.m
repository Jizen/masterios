...
@interface NBNode : NSObject

@property (assign, nonatomic) NSRange sourceRange;
@property (weak, nonatomic) NBNode *parent;
@property (weak, nonatomic) NBNode *nextSibling;

- (NSString*)displayString:(NSString*)pattern;

@end

@implementation NBNode
...
@interface WTReAlternation : NBNode
...
@interface WTReQuantifier : NBNode

- (id)initWithFrom:(NSUInteger)from to:(NSUInteger)to;

@property (strong, nonatomic) NBNode *child;
...
@interface WTReCharacterBase : NBNode
...
- (NSString*)nodeDescription:(NBNode*)node withLevel:(NSUInteger)level
{
    ...
    {
        for (NBNode* c in [(WTReAlternation*)node children])
            [buffer appendString:[self nodeDescription:c withLevel:level+1]];
    }
    else if ([node isKindOfClass:[NBGroup class]])
    {
        for (NBNode* c in [(NBGroup*)node children])
            [buffer appendString:[self nodeDescription:c withLevel:level+1]];
    }
...
- (NBGroup*)groupFromNodes:(NSArray*)nodes enclosed:(BOOL)enclosed
{
    if (nodes.count == 1) {
        NBGroup *t = [nodes objectAtIndex:0];
        if ([t isKindOfClass:[NBGroup class]]) {
            t.capturing |= enclosed;
            return t;
        }
    }
    
    NBGroup *g = [[NBGroup alloc] init];
    g.children = [nodes copy];
    g.capturing = enclosed;
    
    // setup links
    NBNode *prev = [g.children objectAtIndex:0];
    prev.parent = g;
    
    for (NSUInteger i = 1; i < g.children.count; i++) {
        NBNode *curr = [g.children objectAtIndex:i];
        curr.parent = g;
...
- (NBGroup*)parseSubpattern:(NSString*)pattern inRange:(NSRange)range enclosed:(BOOL)enclosed
{
    ...
    for (NSUInteger i = 0; i < range.length; i++) {
        ...
        if (escape) {
            ...
            NBGroup *g = [self groupFromNodes:nodes enclosed:enclosed];
    ...
    if (alternations != nil) {
        ...
        // setup links
        NBNode *prev = [alternations objectAtIndex:0];
        prev.parent = a;
        
        for (NSUInteger i = 1; i < alternations.count; i++) {
            NBNode *curr = [alternations objectAtIndex:i];
            curr.parent = a;
            prev.nextSibling = curr;
            prev = curr;
        }
        
        g = [[NBGroup alloc] init];
    ...
    @autoreleasepool {
        ...
        switch (_regEx) {
            ...
            case REGEX_CONTACT_TYPE1:
                ...
                tInputLen = 13;
                break;
            case REGEX_NUMBER_TYPE:
                [tInput setString:[tInput stringByReplacingOccurrencesOfString:@"-"
                           withString:@""]];
                break;
            default:
                break;
        ...
        if (isMatch) {
            if ([self unicodeLengthOfString:tInput] >= tInputLen) {
                return nil;
            }
        }
...
-(NSUInteger) unicodeLengthOfString:(NSString*)text {
    ...
    NSLog(@"asciiLength = %lu",(unsigned long)asciiLength);
    return asciiLength;
}

- (WTState*)processNode:(NBNode*)node fromState:(WTState*)state length:(NSUInteger)length
{
